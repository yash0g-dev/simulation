...
def stripe_color_at(x, y, stripe_width):
    stripe_index = int((x - y + WIDTH + HEIGHT) // stripe_width) % len(STRIPE_COLORS)
...
def make_ball_texture(radius):
    ...
    for y in range(size):
        for x in range(size):
            texture.set_at((x, y), stripe_color_at(x, y, stripe_width))
...
def paint_stroke(start, end, radius):
    ...
    for y in range(height):
        for x in range(width):
            stripe_surface.set_at((x, y), stripe_color_at(min_x + x, min_y + y, stripe_width))
...
while running:
    ...
    if game_state == "PLAYING":
        ...
        if distance >= inner_limit:
            ...
            if bounce_sounds and (last_sound_time is None or current_time - last_sound_time > 35):
